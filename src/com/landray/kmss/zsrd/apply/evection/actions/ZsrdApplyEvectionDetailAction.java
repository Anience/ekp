...
public class ZsrdApplyEvectionDetailAction extends DataAction {
	...
	public ActionForward data(ActionMapping mapping, ActionForm form,
		...
		IZsrdApplyEvectionService zsrdApplyEvectionService = (IZsrdApplyEvectionService) getServiceImp(request);
		...
		try {
			...
			if(adminman.equals("1183b0b84ee4f581bba001c47a78b2d9".toString()))
			{
				stringBuffer.append("SELECT\n" + "	e.fd_id,\n"
						...
						+ "	(select p.fd_name from sys_org_element p  where e.fd_position = p.fd_id) as fdPosition\n" + " FROM\n"
						+ " zsrd_apply_evection e, \n"
						+ "sys_org_element p \n" + " Where \n"
						+ " e.doc_creator_id = p.fd_id \n"
						+ "ORDER BY e.doc_create_time DESC"
						);
			}
				...
				stringBuffer.append("SELECT\n" + "	e.fd_id,\n"
						...
						+ " and  e.fd_id = w.fd_process_id  \n"
						+ " and e.doc_creator_id = p.fd_id \n"
						+ "UNION \n" + "SELECT \n" + "	e.fd_id,\n"
						+ "	e.fd_to_place, \n" + "	e.fd_vehicle, \n"
						+ "	p.fd_name creatorName, \n" 
						+ "(select p.fd_name from sys_org_element p  where e.fd_position = p.fd_id) as fdPosition \n" + " FROM \n"
						+ " zsrd_apply_evection e, \n" + "sys_org_element p \n" + " Where \n"
						+ "e.doc_creator_id = '" + UserUtil.getUser().getFdId().toString() + "'\n"
						+ "and e.doc_creator_id = p.fd_id \n"
						);
			...
			Page page1 = zsrdApplyEvectionService.listch(hqlInfo);
