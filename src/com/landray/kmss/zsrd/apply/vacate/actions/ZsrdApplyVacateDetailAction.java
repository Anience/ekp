...
public class ZsrdApplyVacateDetailAction extends DataAction {
	...
	public ActionForward data(ActionMapping mapping, ActionForm form,
		...
		IZsrdApplyVacateService zsrdApplyVacateService = (IZsrdApplyVacateService) getServiceImp(request);
		...
		try {
			...
			if(adminman.equals("1183b0b84ee4f581bba001c47a78b2d9".toString()))
			{
				stringBuffer.append("SELECT\n" + "	v.fd_id,\n"
						+ "	v.fd_vacate_reason fdVacateReason,\n" + "	v.fd_vacate_type fdVacateType,\n"
						+ "	(select p.fd_name from sys_org_element p  where v.fd_depart = p.fd_id) as fdDepart,\n" + " p.fd_name creatorName,\n"
						+ "	(select p.fd_name from sys_org_element p  where v.fd_position = p.fd_id) as fdPosition\n" + " FROM\n"
						+ " zsrd_apply_vacate v, \n"
						+ "sys_org_element p \n" + " Where \n"
						+ " v.doc_creator_id = p.fd_id  \n"
						+ "ORDER BY v.doc_create_time DESC"
						);
				...
				stringBuffer.append("SELECT\n" + "	v.fd_id,\n"
						...
						+ " and v.fd_id = w.fd_process_id  \n"
						+ " and v.doc_creator_id = p.fd_id \n"
						+ "UNION \n" + "SELECT\n" + "	v.fd_id,\n"
						+ "	v.fd_vacate_reason fdVacateReason,\n" + "	v.fd_vacate_type fdVacateType,\n"
						+ "	(select p.fd_name from sys_org_element p  where v.fd_depart = p.fd_id) as fdDepart,\n" + " p.fd_name creatorName,\n"
						+ "	(select p.fd_name from sys_org_element p  where v.fd_position = p.fd_id) as fdPosition\n" + " FROM\n"
						+ " zsrd_apply_vacate v, \n" + "sys_org_element p \n"
						+ " Where \n" + "v.doc_creator_id = '" + UserUtil.getUser().getFdId().toString() + "'\n"
						+ " and v.doc_creator_id = p.fd_id \n"
						);
			...
			Page page1 = zsrdApplyVacateService.listch(hqlInfo);
