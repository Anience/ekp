...
public class ZsrdApplySealDetailAction extends DataAction {
	...
	public ActionForward data(ActionMapping mapping, ActionForm form,
		...
		IZsrdApplySealService zsrdApplySealService = (IZsrdApplySealService) getServiceImp(request);
		...
		try {
			...
			if(adminman.equals("1183b0b84ee4f581bba001c47a78b2d9".toString()))
			{
				stringBuffer.append("SELECT\n" + "	s.fd_id,\n"
						+ "	s.fd_to_company,\n" + "	s.fd_seal_num,\n"
						+ "	s.fd_remark,\n" + " (select p.fd_name from sys_org_element p  where s.fd_depart = p.fd_id) as fdServiceDepart,\n"
			...
			Page page1 = zsrdApplySealService.listch(hqlInfo);
