...
<template:include ref="default.list">
	...
	<template:replace name="nav">
		<script>
			function addDoc(){
				Com_OpenWindow("${LUI_ContextPath}/zsrd/apply/carrepair/zsrd_apply_carrepair/zsrdApplyCarrepair.do?method=add");
			}
			function delDoc(){
				var values = [];
				...
				seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($, dialog , topic){
					dialog.confirm('<bean:message key="page.comfirmDelete"/>',function(value){
						if(value==true){
							...
							$.post('${LUI_ContextPath}/zsrd/apply/carrepair/zsrd_apply_carrepair/zsrdApplyCarrepair.do?method=deleteall&categoryId=${param.categoryId}',$.param({"List_Selected":values},true),function(data){
								if(del_load!=null){
									del_load.hide();
		...
		<ui:combin ref="menu.nav.create">
			<ui:varParam name="title" value="${ lfn:message('zsrd-apply-carrepair:module.zsrd.apply.carrepair') }" />
		...
		<div class="lui_list_nav_frame">
			 <%-- <ui:accordionpanel>				 
				...
				<ui:content title="${ lfn:message('list.otherOpt') }">
					<ul class='lui_list_nav_list'>
						<li><a href="${LUI_ContextPath }/sys/?module=zsrd/apply/carrepair" target="_blank">${ lfn:message('list.manager') }</a></li>
					</ul>
				</ui:content>
			</ui:accordionpanel> --%>
		</div>
	...
	<template:replace name="content">
		...
		<list:criteria id="criteria1">
			<list:cri-auto modelName="com.landray.kmss.zsrd.apply.carrepair.model.ZsrdApplyCarrepair" 
				property="docCreateTime;fdCarnum" />
		</list:criteria>
					...
					<td align="right">
						<ui:toolbar count="3">
							...
							<c:import url="/sys/introduce/import/sysIntroduceMain_cancelbtn.jsp" charEncoding="UTF-8">
								<c:param name="fdModelName" value="com.landray.kmss.zsrd.apply.carrepair.model.ZsrdApplyCarrepair" />
							</c:import>
							--%>
							<%-- 修改权限 
							<c:import url="/sys/right/import/doc_right_change_button.jsp" charEncoding="UTF-8">
								<c:param name="modelName" value="com.landray.kmss.zsrd.apply.carrepair.model.ZsrdApplyCarrepair" />
							</c:import>
							--%>				 
						</ui:toolbar>
					</td>
		...
		<ui:fixed elem=".lui_list_operation"></ui:fixed>
	 	...
	 	<list:listview id="listview">
			<ui:source type="AjaxJson">
					{url:'/zsrd/apply/carrepair/zsrd_apply_carrepair/zsrdApplyCarrepairDetail.do?method=listChildren&categoryId=${param.categoryId}'}
			</ui:source>
			<!-- 列表视图 -->	
			<list:colTable isDefault="false"
				rowHref="/zsrd/apply/carrepair/zsrd_apply_carrepair/zsrdApplyCarrepair.do?method=view&fdId=!{fdId}"  name="columntable">
				<list:col-checkbox></list:col-checkbox>
				<list:col-serial></list:col-serial>
				 
				<list:col-auto props="fdReason;fdCarnum;docCreator;fdPrecost;docCreateTime"></list:col-auto>
			</list:colTable>
		</list:listview> 
