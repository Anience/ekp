...
<template:include ref="default.list">
	...
	<template:replace name="nav">
		<script>
			function addDoc(){
				Com_OpenWindow("${LUI_ContextPath}/zsrd/apply/wine/zsrd_apply_wineMg/zsrdApplyWinemg.do?method=add");
			}
			function delDoc(){
				var values = [];
				...
				seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($, dialog , topic){
					dialog.confirm('<bean:message key="page.comfirmDelete"/>',function(value){
						if(value==true){
							...
							$.post('${LUI_ContextPath}/zsrd/apply/wine/zsrd_apply_wineMg/zsrdApplyWinemg.do?method=deleteall&categoryId=${param.categoryId}',$.param({"List_Selected":values},true),function(data){
								if(del_load!=null){
									del_load.hide();
	...
	<template:replace name="content">
		...
		<list:criteria id="criteria1">
			<list:cri-auto modelName="com.landray.kmss.zsrd.apply.wine.model.ZsrdApplyWinemg" 
				property="fdTypename" />
		</list:criteria>
					...
					<td align="right">
						<ui:toolbar count="3">
							...
							<c:import url="/sys/introduce/import/sysIntroduceMain_cancelbtn.jsp" charEncoding="UTF-8">
								<c:param name="fdModelName" value="com.landray.kmss.zsrd.apply.wine.model.ZsrdApplyWinemg" />
							</c:import>
							--%>
							<%-- 修改权限 
							<c:import url="/sys/right/import/doc_right_change_button.jsp" charEncoding="UTF-8">
								<c:param name="modelName" value="com.landray.kmss.zsrd.apply.wine.model.ZsrdApplyWinemg" />
							</c:import>
							--%>				 
						</ui:toolbar>
					</td>
		...
		<ui:fixed elem=".lui_list_operation"></ui:fixed>
	 	...
	 	<list:listview id="listview">
			<ui:source type="AjaxJson">
					{url:'/zsrd/apply/wine/zsrd_apply_wineMg/zsrdApplyWinemg.do?method=data&categoryId=${param.categoryId}'}
			</ui:source>
			<!-- 列表视图 -->	
			<list:colTable isDefault="false"
				rowHref="/zsrd/apply/wine/zsrd_apply_wineMg/zsrdApplyWinemg.do?method=view&fdId=!{fdId}"  name="columntable">
				<list:col-checkbox></list:col-checkbox>
				<list:col-serial></list:col-serial>
				 
				<list:col-auto props="fdTypename;fdPrice;fdNum;fdPer"></list:col-auto>
			</list:colTable>
		</list:listview> 
