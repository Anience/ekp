...
<template:include ref="default.list">
	...
	<template:replace name="nav">
		<script>
			function addDoc(){
				Com_OpenWindow("${LUI_ContextPath}/zsrd/knc/workofplan/zsrd_knc_work_of_plan/zsrdKncWorkOfPlan.do?method=add");
			}
			function delDoc(){
				var values = [];
				...
				seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($, dialog , topic){
					dialog.confirm('<bean:message key="page.comfirmDelete"/>',function(value){
						if(value==true){
							...
							$.post('${LUI_ContextPath}/zsrd/knc/workofplan/zsrd_knc_work_of_plan/zsrdKncWorkOfPlan.do?method=deleteall&categoryId=${param.categoryId}',$.param({"List_Selected":values},true),function(data){
								if(del_load!=null){
									del_load.hide();
		...
		<ui:combin ref="menu.nav.create">
			<ui:varParam name="title" value="${ lfn:message('zsrd-knc-workofplan:module.zsrd.knc.workofplan') }" />
			<ui:varParam name="button">
				[
					<kmss:auth requestURL="/zsrd/knc/workofplan/zsrd_knc_work_of_plan/zsrdKncWorkOfPlan.do?method=add">
					{
						"text": "${ lfn:message('zsrd-knc-workofplan:module.zsrd.knc.workofplan') }",
		...
		<div class="lui_list_nav_frame">
			 <ui:accordionpanel>				 
				...
				<ui:content title="${ lfn:message('list.otherOpt') }">
					<ul class='lui_list_nav_list'>
						<li><a href="${LUI_ContextPath }/sys/?module=zsrd/knc/workofplan" target="_blank">${ lfn:message('list.manager') }</a></li>
					</ul>
				</ui:content>--%>
			</ui:accordionpanel>
		</div>
	...
	<template:replace name="content">
		...
		<list:criteria id="criteria1">
			...
			<list:cri-auto modelName="com.landray.kmss.zsrd.knc.workofplan.model.ZsrdKncWorkOfPlan" 
				property="fdWorkName" />
		</list:criteria>
					...
					<td align="right">
						<ui:toolbar count="3">
							...
							<c:import url="/sys/introduce/import/sysIntroduceMain_cancelbtn.jsp" charEncoding="UTF-8">
								<c:param name="fdModelName" value="com.landray.kmss.zsrd.knc.workofplan.model.ZsrdKncWorkOfPlan" />
							</c:import>
							--%>
							<%-- 修改权限
							<c:import url="/sys/right/import/doc_right_change_button.jsp" charEncoding="UTF-8">
								<c:param name="modelName" value="com.landray.kmss.zsrd.knc.workofplan.model.ZsrdKncWorkOfPlan" />
							</c:import>
							--%>
						</ui:toolbar>
					</td>
		...
		<ui:fixed elem=".lui_list_operation"></ui:fixed>
	 	...
	 	<list:listview id="listview">
			<ui:source type="AjaxJson">
					{url:'/zsrd/knc/workofplan/zsrd_knc_work_of_plan/zsrdKncWorkOfPlan.do?method=data&categoryId=${param.categoryId}'}
			</ui:source>
			<!-- 列表视图 -->	
			<list:colTable isDefault="false"
				rowHref="/zsrd/knc/workofplan/zsrd_knc_work_of_plan/zsrdKncWorkOfPlan.do?method=view&fdId=!{fdId}"  name="columntable">
				<list:col-checkbox></list:col-checkbox>
				<list:col-serial></list:col-serial>
				 
				<list:col-auto props="fdWorkName;fdWork;fdPerson.fdName"></list:col-auto>
			</list:colTable>
			<!-- 摘要视图 -->	
			<list:rowTable isDefault="false"
				rowHref="/zsrd/knc/workofplan/zsrd_knc_work_of_plan/zsrdKncWorkOfPlan.do?method=view&fdId=!{fdId}" name="rowtable" >
				...
				docSubject;icon;fdDescription;docCategory.fdName;docAuthor.fdName;docDept.fdName;docPublishTime;tag
				 --%>
			   	{"showOtherProps":"fdWorkName;fdTime;fdPerson.fdName"}
				</list:row-template>
			</list:rowTable>
		</list:listview> 
