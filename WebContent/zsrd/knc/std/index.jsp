...
<template:include ref="default.list">
	...
	<template:replace name="path">			
		...
		<ui:combin ref="menu.path.simplecategory">
			<ui:varParams 
				id="simplecategoryId"
				moduleTitle="${ lfn:message('zsrd-knc-std:module.zsrd.knc.std') }" 
				href="/zsrd/knc/std/" 
	...
	<template:replace name="nav">
		...
		window.addDoc = function() {
			seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($, dialog , topic){
				dialog.simpleCategoryForNewFile(
						...
						'/zsrd/knc/std/zsrd_knc_std_main/zsrdKncStdMain.do?method=add&docCategoryId=!{id}',false,null,null,'${JsParam.categoryId}');
			});
			};
			function delDoc(){
				var values = [];
				...
				seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($, dialog , topic){
					dialog.confirm('<bean:message key="page.comfirmDelete"/>',function(value){
						if(value==true){
							...
							$.post('${LUI_ContextPath}/zsrd/knc/std/zsrd_knc_std_main/zsrdKncStdMain.do?method=deleteall&categoryId=${param.categoryId}',$.param({"List_Selected":values},true),function(data){
								if(del_load!=null){
									del_load.hide();
		...
		<ui:combin ref="menu.nav.create">
			<ui:varParam name="title" value="${ lfn:message('zsrd-knc-std:module.zsrd.knc.std') }" />
			<ui:varParam name="button">
				[
					<kmss:auth requestURL="/zsrd/knc/std/zsrd_knc_std_main/zsrdKncStdMain.do?method=add">
					{
						"text": "${ lfn:message('zsrd-knc-std:module.zsrd.knc.std') }",
		...
		<div class="lui_list_nav_frame">
			 <ui:accordionpanel>				 
				...
				<ul class='lui_list_nav_list'>
					...
					<%-- 分类索引 --%>
				<ui:content style="padding:0px;" title="标准分类">
				    <!--分类树  -->
	...
	<template:replace name="content">
		...
		<list:criteria id="criteria1">
			<list:cri-auto modelName="com.landray.kmss.zsrd.knc.std.model.ZsrdKncStdMain" 
				...
				<tr	>
					...
					<td align="right">
						<ui:toolbar count="3">
							...
							<kmss:auth requestURL="/zsrd/knc/std/zsrd_knc_std_main/zsrdKncStdMain.do?method=deleteall">					 								
							...
							<c:import url="/sys/introduce/import/sysIntroduceMain_cancelbtn.jsp" charEncoding="UTF-8">
								<c:param name="fdModelName" value="com.landray.kmss.zsrd.knc.std.model.ZsrdKncStdMain" />
							</c:import>
							--%>
							<%-- 修改权限 
							<c:import url="/sys/right/import/doc_right_change_button.jsp" charEncoding="UTF-8">
								<c:param name="modelName" value="com.landray.kmss.zsrd.knc.std.model.ZsrdKncStdMain" />
							</c:import>
							--%>				 
						</ui:toolbar>
					</td>
		...
		<ui:fixed elem=".lui_list_operation"></ui:fixed>
	 	...
	 	<list:listview id="listview">
			<ui:source type="AjaxJson">
					{url:'/zsrd/knc/std/zsrd_knc_std_main/zsrdKncStdMainIndex.do?method=listChildren&categoryId=${JsParam.categoryId}'}
			</ui:source>
			<!-- 列表视图 -->	
			<list:colTable isDefault="false"
				rowHref="/zsrd/knc/std/zsrd_knc_std_main/zsrdKncStdMain.do?method=view&fdId=!{fdId}"  name="columntable">
				...
				<list:col-auto props="docSubject;fdDescription;docCreateTime;docAlterTime;fdLastModifiedTime;docPublishTime;fdVersion;fdSize;fdFrom;docCreator.fdName"></list:col-auto>
			</list:colTable>
		</list:listview> 
