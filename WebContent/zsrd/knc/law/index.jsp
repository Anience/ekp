...
<template:include ref="default.list">
	...
	<template:replace name="path">			
		...
		<ui:combin ref="menu.path.simplecategory">
			<ui:varParams 
				id="simplecategoryId"
				moduleTitle="${ lfn:message('zsrd-knc-law:module.zsrd.knc.law') }" 
				href="/zsrd/knc/law/" 
				modelName="com.landray.kmss.zsrd.knc.law.model.ZsrdKncLawCategory" 
	...
	<template:replace name="nav">
		...
		window.addDoc = function() {
			seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($, dialog , topic){
				dialog.simpleCategoryForNewFile(
						...
						'/zsrd/knc/law/zsrd_knc_law_main/zsrdKncLawMain.do?method=add&docCategoryId=!{id}',false,null,null,'${JsParam.categoryId}');
			});
			};
			function delDoc(){
				var values = [];
				...
				seajs.use(['lui/jquery','lui/dialog','lui/topic'], function($, dialog , topic){
					dialog.confirm('<bean:message key="page.comfirmDelete"/>',function(value){
						if(value==true){
							...
							$.post('${LUI_ContextPath}/zsrd/knc/law/zsrd_knc_law_main/zsrdKncLawMain.do?method=deleteall&categoryId=${param.categoryId}',$.param({"List_Selected":values},true),function(data){
								if(del_load!=null){
									del_load.hide();
		...
		<ui:combin ref="menu.nav.create">
			<ui:varParam name="title" value="${ lfn:message('zsrd-knc-law:module.zsrd.knc.law') }" />
			<ui:varParam name="button">
				[
					<kmss:auth requestURL="/zsrd/knc/law/zsrd_knc_law_main/zsrdKncLawMain.do?method=add">
					{
						"text": "${ lfn:message('zsrd-knc-law:module.zsrd.knc.law') }",
		...
		<div class="lui_list_nav_frame">
			 <ui:accordionpanel>				 
				...
				<ul class='lui_list_nav_list'>
					...
					<%-- 分类索引 --%>
				<ui:content style="padding:0px;" title="法律法规分类">
				    <!--分类树  -->
				...
				<%-- <ui:content title="${ lfn:message('list.otherOpt') }">
					<ul class='lui_list_nav_list'>
						<li><a href="${LUI_ContextPath }/sys/?module=zsrd/knc/law" target="_blank">${ lfn:message('list.manager') }</a></li>
					</ul>
				</ui:content> --%>
			</ui:accordionpanel>
		</div>
	...
	<template:replace name="content">
		...
		<list:criteria id="criteria1">
			<list:cri-auto modelName="com.landray.kmss.zsrd.knc.law.model.ZsrdKncLawMain" 
				...
				<tr	>
					...
					<td align="right">
						<ui:toolbar count="3">
							...
							<kmss:auth requestURL="/zsrd/knc/law/zsrd_knc_law_main/zsrdKncLawMain.do?method=deleteall">					 								
							...
							<c:import url="/sys/introduce/import/sysIntroduceMain_cancelbtn.jsp" charEncoding="UTF-8">
								<c:param name="fdModelName" value="com.landray.kmss.zsrd.knc.law.model.ZsrdKncLawMain" />
							</c:import>
							--%>
							<%-- 修改权限 
							<c:import url="/sys/right/import/doc_right_change_button.jsp" charEncoding="UTF-8">
								<c:param name="modelName" value="com.landray.kmss.zsrd.knc.law.model.ZsrdKncLawMain" />
							</c:import>
							--%>				 
						</ui:toolbar>
					</td>
		...
		<ui:fixed elem=".lui_list_operation"></ui:fixed>
	 	...
	 	<list:listview id="listview">
			<ui:source type="AjaxJson">
					{url:'/zsrd/knc/law/zsrd_knc_law_main/zsrdKncLawMainIndex.do?method=listChildren&categoryId=${JsParam.categoryId}'}
			</ui:source>
			<!-- 列表视图 -->	
			<list:colTable isDefault="false"
				rowHref="/zsrd/knc/law/zsrd_knc_law_main/zsrdKncLawMain.do?method=view&fdId=!{fdId}"  name="columntable">
